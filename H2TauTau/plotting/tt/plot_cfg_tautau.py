...
from CMGTools.H2TauTau.proto.plotter.categories_TauTau import cat_Inc
...
inc_cut += '&& l2_decayModeFinding > 0.5 && l1_decayModeFinding > 0.5'
...
samples_mc, samples_data, all_samples = createSampleLists(analysis_dir=analysis_dir,channel='tt')

if qcd_from_same_sign:
    samples_qcdfromss = [s for s in all_samples if s.name != 'QCD']
    #samples_qcdfromss = [s for s in samples_mc if s.name != 'QCD']
...
for cut in cuts:
    if qcd_from_same_sign and not 'SS' in cut.name:
        ...
        #cfg_example = HistogramCfg(name=cut.name, var=None, cfgs=samples_mc, cut=inc_cut, lumi=int_lumi, weight=total_weight)
    ...
    for variable in variables:
        ...
        plot.Group('VV', ['ZZTo2L2Q', 'WWTo1L1Nu2Q', 'WZTo1L3Nu', 'ZZTo4L_new', 'WZTo3L', 'WZTo2L2Q', 'WZTo1L1Nu2Q'])#, 'T_tWch', 'TBar_tWch', 'TToLeptons_tch_powheg', 'TBarToLeptons_tch_powheg'])
